...
class Game:
        ...
        try:
            with open(map_filename, 'r') as file:
                ...
                for room in self.map:
                    room['exits'] = {direction: int(room_id) for direction, room_id in room['exits'].items()}
        except FileNotFoundError:
            print(f"Map file '{map_filename}' not found.")
            sys.exit(1)
        except json.JSONDecodeError:
            print(f"Invalid JSON format in map file '{map_filename}'.")
            sys.exit(1)
    ...
    def go(self, direction):
        if not direction:
            print("You need to enter a direction.")
            return
        ...
        if len(matching_exits) == 1:
            ...
            print(f"Did you want to go {', '.join(matching_exits)}?")
        else:
