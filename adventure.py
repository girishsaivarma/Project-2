...
class Game:
    ...
    def locked_door(self, required_item):
        room_info = self.map[self.current_room]
        if 'locked' in room_info and room_info['locked']:
            if required_item in self.inventory:
                print(f"You unlock the door with {required_item}.")
                room_info['locked'] = False
            else:
                print(f"Error: The door is locked. You need {required_item} to unlock it.")
                return True  # Door is locked, and action failed
        return False  # Door is either not locked or action succeeded

    def win_condition(self):
        if self.current_room == 3 and 'rose' in self.inventory:
...
def main():
    ...
    while True:
        ...
        if verb == 'go':
            ...
            game.quit()
        elif verb in ['north', 'south', 'east', 'west', 'northeast', 'northwest', 'southeast', 'southwest']:
            game.go(verb)
        elif verb == 'unlock':
            if len(user_input) > 1:
                game.locked_door(user_input[1])
        else:
